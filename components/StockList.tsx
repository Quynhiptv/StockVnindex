...
const StockList: React.FC = () => {
  ...
  const alertHistory = useRef<Record<string, { lastSent: number, count: number, date: string }>>(
    JSON.parse(localStorage.getItem('stock_alerts_history') || '{}')
  );
  ...
  const updateHistory = (key: string, state: { lastSent: number, count: number, date: string }) => {
    ...
    localStorage.setItem('stock_alerts_history', JSON.stringify(alertHistory.current));
  };

  const sendAlert = (stock: StockListItem, type: 'watch' | 'buy') => {
    let message = '';
